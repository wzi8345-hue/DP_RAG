...
【中文问句 kw 硬约束 (跨语言检索)】
- 用户发话含汉字 → paths[].kw **必须**同时包含中文主题/问点词 **和** 英文同义词/术语/缩写
  (至少 1 组中英成对, 如 点蚀/pitting corrosion, 循环寿命/cycle life, 钒电池/vanadium battery).
- 同时设 `rerank_mode=true`, 让精排用 kw (含英文锚点) 而非中文原话.
- 纯英文问句: 保留英文术语即可, 可酌情补中文领域词.
...
- **ctype=references 触发门槛 (严)**: 仅当用户**显式索取引文列表本身** (措辞含 "参考文献 /
  引用文献 / 引文列表 / 引用了哪些文献 / references / bibliography / works cited") 才填.
  ...
  【严禁误判】句中只是提到某标准/方法/试验/规范名 (如 "用哪个 ASTM 标准方法表征了 X"、
  "依据什么测试方法"、"standard test method") **不是**问参考文献 — 这是正文事实问句,
  走 progressive/local 且**不带 ctype**; 不要因为出现 "standard/method/reference" 等词就填 ctype=references.
...
24. 用户: "Which ASTM standard test method was used to characterize pitting corrosion susceptibility for the Nitinol ocular device"
                                                         → plan(paths=[{t=progressive, kw=["ASTM","standard test method","pitting corrosion","点蚀","Nitinol","NiTi"]}])
                                                            (⚠️ 提到 "ASTM standard / test method" **不等于**问参考文献; 这是正文事实问句 → progressive, **不带 ctype=references**)
25. 用户: "Nitinol 眼科器械用了哪种 ASTM 标准方法表征点蚀敏感性"
                                                         → plan(paths=[{t=progressive, kw=["Nitinol","NiTi","眼科","ocular device","ASTM","standard test method","点蚀","pitting corrosion","susceptibility"]}], rerank_mode=true)
                                                            (中文问句 → kw 必须中英成对 + rerank_mode=true)
