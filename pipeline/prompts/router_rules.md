...
4. **按路径的 kw 配方 (场景约束)**
   - **【硬约束】中文问句必须带英文同义词**: 用户发话含汉字时, kw **必须**同时包含
     中文主题/问点词 **和** 对应英文术语/缩写/化学式 (至少 1 组中英成对).
     动机: 语料多为英文, BM25 与 dense 都依赖英文锚点; 只写中文 kw 会严重漏召回.
     同时应设 `rerank_mode=true` (精排用 kw 而非中文原话).
     例: 点蚀腐蚀 → ["点蚀","pitting corrosion","ASTM","standard test method"]
   ...
   - **progressive (开放域事实)**: **主体词 + 问点属性词** 成对出现; 数值/单位/Max./最小 等问点保留;
     材料领域加中英成对 (碳/C, 循环寿命/cycle life); **中文问句必须补英文同义词** (见上).
     例: 试样规格 → ["试样","specimen","规格","dimensions","50mm"]
